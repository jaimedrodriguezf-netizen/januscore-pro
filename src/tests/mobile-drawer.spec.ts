...
test.describe('Mobile Navigation Drawer Redesign', () => {
  ...
  test('drawer should have glassmorphism styling', async ({ page }) => {
    ...
    const backdropFilter = await drawer.evaluate(
  ...
  test('drawer should contain professional contact information', async ({
    page,
  }) => {
  ...
  test('drawer should contain a prominent WhatsApp button', async ({
    page,
  }) => {
  ...
  test('drawer should contain social media links with icons', async ({
    page,
  }) => {
    const socialLinks = page.locator(
      'nav[role="dialog"] .social-links-footer a',
    );
  ...
  test('menu items should be structured with hierarchy', async ({ page }) => {
    ...
    const contactHeader = page.locator(
      'nav[role="dialog"] span:text-is("Contacto")',
    );
